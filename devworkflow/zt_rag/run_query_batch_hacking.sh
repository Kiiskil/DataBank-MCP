#!/usr/bin/env bash
# ZT-RAG-testikysymykset — Databank Hacking (10 kpl).
# Kysymykset: devworkflow/zt_rag/query_batch_questions_hacking.txt
# Lämmin MCP: python -m devworkflow.zt_rag.mcp_query_batch + env alla
# Käyttö: ./devworkflow/zt_rag/run_query_batch_hacking.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
IMG="${ZT_RAG_IMAGE:-localhost/datapankki-mcp:latest}"
VOL="${ZT_RAG_VOLUME:-zt-rag-data-databank-hacking}"
BANK="${ZT_DATABANK:-$ROOT/Databank/Hacking}"
QFILE="${ZT_QUERY_BATCH_FILE:-$ROOT/devworkflow/zt_rag/query_batch_questions_hacking.txt}"
