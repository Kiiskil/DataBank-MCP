#!/usr/bin/env bash
# ZT-RAG-testikysymykset Podmanissa (10 kpl: 5 perus + 5 vaikeaa).
# Kysymykset: devworkflow/zt_rag/query_batch_questions.txt
# Lämmin MCP (yksi palvelinprosessi): python -m devworkflow.zt_rag.mcp_query_batch
# Käyttö: ./devworkflow/zt_rag/run_query_batch.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
IMG="${ZT_RAG_IMAGE:-localhost/datapankki-mcp:latest}"
VOL="${ZT_RAG_VOLUME:-zt-rag-data-databank-ai}"
BANK="${ZT_DATABANK:-$ROOT/Databank/AI}"
QFILE="${ZT_QUERY_BATCH_FILE:-$ROOT/devworkflow/zt_rag/query_batch_questions.txt}"
...
if not candidates:
    ...
    sys.exit(1)
# Suurin objekti = tyypillisesti koko zt_query-vastaus (ei sisäkkäisiä pieniä dictejä)
