#!/usr/bin/env bash
# ZT-RAG-testikysymykset — Databank Software (10 kpl).
# Kysymykset: devworkflow/zt_rag/query_batch_questions_software.txt
# Lämmin MCP (yksi Podman + stdio): python -m devworkflow.zt_rag.mcp_query_batch + env alla
# Käyttö: ./devworkflow/zt_rag/run_query_batch_software.sh
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
IMG="${ZT_RAG_IMAGE:-localhost/datapankki-mcp:latest}"
VOL="${ZT_RAG_VOLUME:-zt-rag-data-databank-software}"
BANK="${ZT_DATABANK:-$ROOT/Databank/Software}"
QFILE="${ZT_QUERY_BATCH_FILE:-$ROOT/devworkflow/zt_rag/query_batch_questions_software.txt}"
